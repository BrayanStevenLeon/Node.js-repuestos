...
async function eliminarUsuario(id) {
    if (confirm("¿Estás seguro de eliminar este usuario?")) {
        await fetch(`http://127.0.0.1:4000/api/users/${id}`, { method: 'DELETE',credentials: 'include' });
        cargarUsuarios(); 
    }
}
...
async function editarUsuario(id) {
    ...
    if (nuevoNombre && nuevoEmail && nuevoRol) {
        ...
        await fetch(`http://127.0.0.1:4000/api/users/${id}`, {
            method: 'PUT',
            credentials: 'include',
            headers: { 'Content-Type': 'application/json' },
...
document.getElementById('logoutButton').addEventListener('click', async function() {
    try {
        const response = await fetch('http://127.0.0.1:4000/api/logout', { method: 'POST', credentials: 'include'
         });
