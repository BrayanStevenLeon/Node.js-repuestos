      ...
      if (Array.isArray(data)) {
        ...
        ventasFiltradas.forEach(venta => {
          ...
          if (venta.estado_pago === 'pendiente') {
            ...
            botonAccion = `
              <div style="display: flex; gap: 10px;">
                <a href="http://localhost:4000/${venta.factura_pdf}" target="_blank" download class="btn-descargar-factura" title="Descargar factura">
...
document.addEventListener('click', async e => {
  ...
  if (e.target.closest('.btn-pagar')) {
    ...
    try {
      const res = await fetch(`http://localhost:4000/api/ventas/pagar/${idVenta}`, {
        method: 'PUT',
  ...
  if (e.target.closest('.btn-cancelar')) {
    ...
    try {
      const res = await fetch(`http://localhost:4000/api/ventas/cancelar/${idVenta}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ usuario_id: usuario.id })
      });
  ...
  if (e.target.closest('.btn-detalle')) {
    ...
    try {
      const res = await fetch(`http://localhost:4000/api/ventas/detalle/${idVenta}`);
...
function cerrarSesion() {
  fetch('http://localhost:4000/api/logout', { method: 'POST' })
    .then(r => {
